...
The introduction of the \textbf{Transformer} architecture \autocite{ar:attention} and its extension to computer vision via the \textit{\gls{ViT}} \autocite{ar:dosovitskiy-2020} dismantled the locality constraints of \textit{\glspl{CNN}}. By utilizing the global self-attention mechanism \autocite{ar:attention}, the \textit{Transformer} models the relationship between input components in a sequence with a constant path length of $\mathcal{O}\left(1\right)$, regardless of their physical distance or sensory modality.

This transition from local feature modeling to global sequence modeling enabled the emergence of \textit{\glspl{MFM}} \autocite{ar:bommasani-2021} in the physical sciences. Instead of treating images and spectra as isolated and independent datasets, contemporary computational astrophysics architectures project them into a shared latent space. To unify disparate astronomical data, \textit{\glspl{MFM}} transform multiband images, dispersed spectra, and tabular metadata into homogeneous sequences of vectors called tokens \autocites{ar:astropt}{ar:astroclip}{ar:aion-1}. In the case of astronomical images, these are divided into local patches (for example, of $16 \times 16$ pixels), linearly projecting each patch to a continuous representation space. To avoid ambiguity, it is important to understand the hierarchy between \textbf{patch} and \textbf{token}: a \textit{patch} is a discrete two-dimensional spatial or spectral subregion cropped from the original observation (for example, a matrix of $16 \times 16$ pixels of astronomical flux from an image), whereas a \textit{token} is the abstract vector representation of that patch resulting from flattening it and projecting it through the encoder into the embedding space of the \textit{Transformer} model.
